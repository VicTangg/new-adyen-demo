...
ADYEN_MANAGEMENT_WRITE_TOKEN=choose_a_long_random_management_write_token
IMAGE_HOST_DELETE_TOKEN=choose_a_long_random_image_delete_token
```
...
- **ADYEN_MANAGEMENT_WRITE_TOKEN**: Required for PATCH requests that modify store or split-configuration settings. Send it as `Authorization: Bearer ...` or `X-Adyen-Management-Write-Token`.
- **IMAGE_HOST_DELETE_TOKEN**: Required for deleting all hosted images. Send it as `Authorization: Bearer ...` or `X-Image-Host-Delete-Token`.
