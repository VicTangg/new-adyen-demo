...
# Required only if you intentionally enable Adyen Management API write proxy routes.
# ADYEN_MANAGEMENT_WRITE_TOKEN=change_me_to_a_random_operator_token
```
...
- **ADYEN_MANAGEMENT_WRITE_TOKEN**: Optional operator token for Management API write routes. If unset, store/split-configuration edits are rejected server-side.
