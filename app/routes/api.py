...
def _require_image_delete_token():
    return _configured_token_required("IMAGE_HOST_DELETE_TOKEN", "X-Image-Host-Delete-Token")


def _require_adyen_management_write_token():
    return _configured_token_required("ADYEN_MANAGEMENT_WRITE_TOKEN", "X-Adyen-Management-Write-Token")
...
def _checkout_amount(payment_method=None):
    ...
    return {"value": value, "currency": CHECKOUT_CURRENCY}
...
def _checkout_success_url():
    return f"{request.url_root.rstrip('/')}/checkout/success"


def _checkout_failed_url():
    return f"{request.url_root.rstrip('/')}/checkout/failed"


def _xendit_session_origins():
    base_url = request.url_root.rstrip("/")
    base_https = base_url if base_url.startswith("https://") else base_url.replace("http://", "https://", 1)
    origins = [base_https, "https://localhost:5001", "https://new-adyen-demo-377583b87f3c.herokuapp.com"]
...
def image_host_upload():
    ...
    # Reject the new upload if it would exceed the cap; never delete existing links.
    if _image_host_is_over_limit():
        file_path.unlink(missing_ok=True)
        return jsonify({
            "error": "stored files would exceed 100 MB; upload rejected"
        }), 507
...
def image_host_delete_all():
    ...
    auth_error = _require_image_delete_token()
    if auth_error:
        return auth_error
...
def adyen_payments():
    ...
    data = copy.deepcopy(request.get_json() or {})
    if not current_app.config.get("ADYEN_MERCHANT_ACCOUNT") or not current_app.config.get("ADYEN_API_KEY"):
        return jsonify({"error": "Adyen not configured"}), 503

    # Payment-critical fields must come from the server-side cart/config, not the browser.
    ...
    data["channel"] = "Web"
    data["origin"] = request.url_root.rstrip("/")
...
def xendit_payment_request():
    ...
    result, err = _do_xendit_payment_request(
        channel_code, None, _checkout_success_url(), _checkout_failed_url()
    )
    if err:
        return err[0], err[1]
...
def xendit_payment_grabpay():
    ...
    result, err = _do_xendit_payment_request("GRABPAY", None, _checkout_success_url(), _checkout_failed_url())
    if err:
        return err[0], err[1]
    redirect_url, payment_id = result
    return jsonify({"redirect_url": redirect_url, "payment_request_id": payment_id})
