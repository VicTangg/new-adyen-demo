...
def _require_token(config_key, header_name):
    expected = str(current_app.config.get(config_key, "") or "").strip()
    if not expected:
        return jsonify({"error": f"{config_key} is not configured"}), 403
    supplied = (request.headers.get(header_name) or _bearer_token()).strip()
...
def _configured_origins(config_key):
    raw = current_app.config.get(config_key, "") or ""
    return [origin.strip().rstrip("/") for origin in raw.split(",") if origin.strip()]


def _checkout_amount_for_payment(payment_method=None):
    from app.routes.pages import CHECKOUT_CURRENCY, get_checkout_total_cents

    amount = get_checkout_total_cents()
    method_type = (payment_method or {}).get("type")
    if method_type == "alipay":
        amount = round(amount * ALIPAY_DISCOUNT_RATE)
    return {"value": int(amount), "currency": CHECKOUT_CURRENCY}
...
def image_host_upload():
    ...
    # Enforce the hard cap without deleting existing hosted images.
...
def image_host_delete_all():
    ...
    auth_error = _require_token("IMAGE_HOST_DELETE_TOKEN", "X-Image-Host-Delete-Token")
    if auth_error:
        return auth_error
...
def adyen_payment_methods():
    ...
    params = {
        ...
        "amount": _checkout_amount_for_payment(),
...
def adyen_payments():
    ...
    # Money and routing fields are server-owned; client-supplied values can undercharge or redirect off-site.
    data["amount"] = _checkout_amount_for_payment(data.get("paymentMethod"))
    ...
    data["channel"] = "Web"
    data["origin"] = _canonical_https_origin()
...
def adyen_store_update(store_id):
    ...
    auth_error = _require_token("ADYEN_MANAGEMENT_WRITE_TOKEN", "X-Adyen-Management-Write-Token")
    if auth_error:
        return auth_error
...
def xendit_create_session():
    ...
    data = request.get_json() or {}
    amount = int(current_app.config.get("XENDIT_SESSION_AMOUNT", 9498))
    ...
    origins = [
        ...
        *_configured_origins("XENDIT_ALLOWED_ORIGINS"),
...
def xendit_payment_request():
    ...
    failure_url = _checkout_failed_url()
