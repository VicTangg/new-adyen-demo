...
def _request_auth_token(header_name):
...
def _require_configured_token(config_key, header_name, label):
    expected = (current_app.config.get(config_key) or "").strip()
    if not expected:
        return jsonify({"error": f"{label} token not configured"}), 503

    supplied = _request_auth_token(header_name)
...
def image_host_upload():
    ...
    # Enforce the storage cap without deleting previously uploaded images.
    if _image_host_total_size_bytes() > IMAGE_HOST_MAX_BYTES:
        file_path.unlink(missing_ok=True)
        return jsonify({
            "error": "stored files would exceed the 100 MB limit; upload was rejected"
        }), 507
