...
def _require_configured_token(config_key, header_name):
    ...
    if not expected:
        return jsonify({"error": f"{config_key} not configured"}), 503

    provided = (request.headers.get(header_name) or "").strip() or _bearer_token()
...
def image_host_upload():
    ...
    if _image_host_total_size_bytes() > IMAGE_HOST_MAX_BYTES:
        ...
        return jsonify({
            "error": "stored files would exceed 100 MB"
        }), 507
...
def adyen_payments():
    ...
    # Sensitive payment fields are authoritative on the server.
...
def xendit_create_session():
    ...
    # Xendit requires HTTPS origins. Do not trust browser-supplied origins.
...
def xendit_payment_request():
    ...
    success_url = _checkout_url("pages.checkout_success")
    failure_url = _checkout_url("pages.checkout_failed")
