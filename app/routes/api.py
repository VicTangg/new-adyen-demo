...
def _configured_token(name):
    token = current_app.config.get(name) or ""
    return token.strip()
...
def _require_token(config_name, header_name):
    expected = _configured_token(config_name)
...
def _adyen_payment_amount(data):
...
def image_host_upload():
    ...
    if _image_host_total_size_bytes() > IMAGE_HOST_MAX_BYTES:
        ...
        return jsonify({
            "error": "stored files exceeded 100 MB; upload was rejected"
        }), 507
...
def adyen_payments():
    ...
    data["amount"] = _adyen_payment_amount(data)
...
def xendit_payment_request():
    ...
    base_url = request.url_root.rstrip("/")
    success_url = f"{base_url}{url_for('pages.checkout_success')}"
    failure_url = f"{base_url}{url_for('pages.checkout_failed')}"
