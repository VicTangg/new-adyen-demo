...
def _require_configured_token(config_key, header_name):
    ...
    supplied = _request_token(header_name)
...
def _checkout_amount():
    ...
    return {"value": get_checkout_total_cents(), "currency": CHECKOUT_CURRENCY}


def _adyen_return_url():
    return f"{request.url_root.rstrip('/')}/checkout/return"


def _xendit_return_urls():
    base_url = request.url_root.rstrip("/")
    return f"{base_url}/checkout/success", f"{base_url}/checkout/failed"
...
def image_host_upload():
    ...
    # Reject the new upload without deleting files that were already hosted.
    if _image_host_purge_if_over_limit():
        file_path.unlink(missing_ok=True)
        return jsonify({
            "error": "stored files exceeded 100 MB; uploaded image was not saved"
        }), 507
...
def adyen_payments():
    ...
    if payment_method.get("type") == "alipay":
        amount["value"] = round(amount["value"] * 0.7)

    data.update({
        "amount": amount,
        "reference": f"ref-{uuid.uuid4().hex[:16]}",
        "returnUrl": _adyen_return_url(),
        "merchantAccount": current_app.config["ADYEN_MERCHANT_ACCOUNT"],
        "channel": "Web",
    })
...
def xendit_create_session():
    ...
    base_https = _https_origin(request.url_root.rstrip("/"))
...
def xendit_payment_request():
    ...
    success_url, failure_url = _xendit_return_urls()
