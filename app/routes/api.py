...
def _require_adyen_management_write_auth():
    """Require a server-configured token before mutating Adyen Management state."""
    expected_token = str(current_app.config.get("ADYEN_MANAGEMENT_WRITE_TOKEN") or "").strip()
    if not expected_token:
        return jsonify({"error": "Adyen Management write API is not configured"}), 403
    ...
    if not provided_token:
        provided_token = request.headers.get("X-Adyen-Management-Write-Token", "").strip()

    if not provided_token or not hmac.compare_digest(provided_token, expected_token):
        return jsonify({"error": "Unauthorized Adyen Management write request"}), 401
    return None
